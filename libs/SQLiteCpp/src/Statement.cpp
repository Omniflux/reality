...
// Compile and register the SQL query for the provided SQLite Database Connection
Statement::Statement(Database &aDatabase, const QString& aQuery) :
    mQuery(aQuery.toUtf8()),
    mStmtPtr(aDatabase.mpSQLite, mQuery), // prepare the SQL query, and ref count (needs Database friendship)
    mColumnCount(0),
    mbOk(false),
    mbDone(false)
...
// Bind a string value to a parameter "?NNN", ":VVV", "@VVV" or "$VVV" in the SQL prepared statement
void Statement::bind(const char* apName, const QString& aValue)
{
  int index = sqlite3_bind_parameter_index(mStmtPtr, apName);
  int ret   = sqlite3_bind_text(mStmtPtr, 
                                index, 
                                aValue.toUtf8(), 
